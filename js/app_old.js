...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  document.querySelectorAll("[data-toast]").forEach(el=>{
	...
	// Keep focused sidebar nav items visible (works for Shift+Tab too)
document.addEventListener('focusin', (e) => {
  const t = e.target;
  if (!t) return;

  // Only care about focus inside the sidebar nav card
  const sidebar = t.closest?.('.sidebar');
  if (!sidebar) return;

  // If the focused element is outside the visible part of the sidebar, scroll it into view
